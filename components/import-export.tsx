...
export default function ImportExport() {
    ...
    const handleExport = () => {
        ...
        const exportFileDefaultName = `jeopardy-game-${new Date()
            .toISOString()
            .slice(0, 10)}.json`;
        ...
        try {
            ...
            parsedData.categories.forEach(
                (category: Category, index: number) => {
                    if (!category.title) {
                        throw new Error(
                            `Category at index ${index} is missing a title`
                        );
                    }
                    ...
                    if (
                        ...
                        throw new Error(
                            `Category "${category.title}" is missing questions array`
                        );
                    }
            ...
            if (err instanceof Error) {
                ...
                setError("Import failed: An unknown error occurred");
            }
        }
    };
        ...
        reader.onload = (event) => {
            try {
                if (event.target && typeof event.target.result === "string") {
                    ...
                    setError("File read error: Invalid file content");
                }
            } catch (err) {
                if (err instanceof Error) {
                    setError(`File read error: ${err.message}`);
                } else {
                    setError("File read error: An unknown error occurred");
                }
            }
        };
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-gray-500 mb-4">
                    Download your current game data as a JSON file that you can
                    save or share.
                </p>
